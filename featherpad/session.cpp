...
void SessionDialog::saveSession() {
    ...
    if (ui->windowBox->isChecked()) {
        ...
        for (int i = 0; i < win->ui->tabWidget->count(); ++i) {
            if (!qobject_cast<TabPage*>(win->ui->tabWidget->widget(i))->textEdit()->getFileName().isEmpty()) {
                hasFile = true;
                break;
            }
        }
        ...
        for (int i = 0; i < singleton->Wins.count(); ++i) {
            ...
            for (int j = 0; j < win->ui->tabWidget->count(); ++j) {
                if (!qobject_cast<TabPage*>(win->ui->tabWidget->widget(j))->textEdit()->getFileName().isEmpty()) {
                    hasFile = true;
                    break;
                }
            }

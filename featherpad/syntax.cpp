    ...
    if (dotExp.match(baseName).hasMatch()) {
        if (fname.endsWith(".cpp") || fname.endsWith(".h"))
            ...
            progLan = "c";
                 ...
                 fname.endsWith(".zsh") || baseName == ".bash_profile" || baseName == ".bash_functions" ||
                 baseName == ".bash_logout" || baseName == ".bash_aliases" || baseName == ".xprofile" ||
                 baseName == ".profile" || baseName == ".mkshrc" || baseName == ".zprofile" || baseName == ".zlogin" ||
            ...
            progLan = "LaTeX";
        else if (fname.endsWith(".xml", Qt::CaseInsensitive) || fname.endsWith(".svg", Qt::CaseInsensitive) ||
                 fname.endsWith(".qrc") || fname.endsWith(".meta4", Qt::CaseInsensitive) ||
                 fname.endsWith(".metalink", Qt::CaseInsensitive) || fname.endsWith(".rdf") ||
                 fname.endsWith(".docbook") || fname.endsWith(".fnx") || fname.endsWith(".ts") ||
                 fname.endsWith(".menu") || fname.endsWith(".kml", Qt::CaseInsensitive) ||
                 fname.endsWith(".xspf", Qt::CaseInsensitive) || fname.endsWith(".asx", Qt::CaseInsensitive) ||
                 fname.endsWith(".nfo") ||
                 fname.endsWith(".dae") /* || fname.endsWith (".ui") || fname.endsWith (".xul")*/)
            progLan = "xml";
            ...
            progLan = "desktop";
        else if (fname.endsWith(".kvconfig") || fname.endsWith(".service") || fname.endsWith(".mount") ||
                 fname.endsWith(".timer")  // systemd related
                 || baseName == "sources.list" || baseName == "sources.list.save" || baseName == "locale.gen" ||
                 baseName == "mimeinfo.cache" || baseName == "defaults.list" || baseName == "mimeapps.list" ||
                 baseName.endsWith("-mimeapps.list") || fname.endsWith(".pls", Qt::CaseInsensitive))
            progLan = "config";
            ...
            progLan = "toml";
        else if (baseName.startsWith("makefile.", Qt::CaseInsensitive) &&
                 !baseName.endsWith(".txt", Qt::CaseInsensitive))
            progLan = "makefile";
    ...
    if (progLan.isEmpty())  // now, check the mime type
    {
        if (!fInfo.exists())
            ...
            if (mime == "text/x-c++" || mime == "text/x-c++src" || mime == "text/x-c++hdr" || mime == "text/x-chdr")
                ...
                progLan = "LaTeX";
            else if (mime == "text/html" || parentMime == "text/html" ||
                     mime == "application/xhtml+xml")  // should come before xml check
                progLan = "html";
            else if (mime == "application/xml" || parentMime == "application/xml" || mime == "text/feathernotes-fnx" ||
                     mime == "audio/x-ms-asx" || mime == "text/x-nfo" || mime == "application/xml-dtd")
                progLan = "xml";
...
void FPwin::syntaxHighlighting(TextEdit* textEdit, bool highlight, const QString& lang) {
    if (textEdit == nullptr || textEdit->isUneditable())  // has huge lines or isn't a text
    {
        return;
    }
    ...
    if (highlight) {
            ...
            QTimer::singleShot(100, textEdit, [=]() {
                if (TabPage* tabPage = qobject_cast<TabPage*>(ui->tabWidget->currentWidget())) {
                    if (tabPage->textEdit() == textEdit)
                        showWarningBar("<center><b><big>" + tr("The size limit for syntax highlighting is exceeded.") +
                                       "</big></b></center>");
                }
            });
            return;
        ...
        if (!qobject_cast<Highlighter*>(textEdit->getHighlighter())) {
            ...
            Highlighter* highlighter =
                new Highlighter(textEdit->document(), progLan, start, end, textEdit->hasDarkScheme(),
                                config.getShowWhiteSpace(), config.getShowEndings(), config.getWhiteSpaceValue(),

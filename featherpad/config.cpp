...
void Config::readConfig() {
    ...
    if (noToolbar_ && noMenubar_) {  // we don't want to hide all actions
        noToolbar_ = false;
        noMenubar_ = true;
    }
...
void Config::readSyntaxColors()  // may be called multiple times
{
    ...
    for (auto& syntax : syntaxes) {
        ...
        if (!col.isValid() || l.contains(col)) {  // an invalid or repeated color shows a corrupted configuration
            customSyntaxColors_.clear();
            break;
        }
        l << col;
...
void Config::writeSyntaxColors() {
    ...
    if (customSyntaxColors_.isEmpty()) {  // avoid redundant writing as far as possible
...
QString Config::validatedShortcut(const QVariant v, bool* isValid) {
    ...
    if (v.isValid()) {
        ...
        if (str.isEmpty()) {  // it means the removal of a shortcut
            *isValid = true;
            return QString();
        }

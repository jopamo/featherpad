...
bool Highlighter::isPascalQuoted(const QString& text, const int index, const int start) const {
    if (index < 0)
        return false;
    int N = 0;
    int indx = start;
    while ((indx = text.indexOf(quoteMark, indx)) > -1 && indx < index) {
        if (format(indx) != commentFormat && format(indx) != urlFormat && format(indx) != regexFormat) {
            ++N;
        }
        ++indx;
    }
...
/*************************/
bool Highlighter::isPascalMLCommented(const QString& text, const int index, const int start) const {
...
void Highlighter::pascalQuote(const QString& text, const int start) {
    ...
    while (index >= 0) {
        ...
        int quoteLength = endIndex - index + quoteMatch.capturedLength();
...
void Highlighter::multiLinePascalComment(const QString& text) {
    ...
    while (startIndex >= 0) {
        ...
        setFormat(startIndex, commentLength, compilerDirective ? regexFormat : commentFormat);

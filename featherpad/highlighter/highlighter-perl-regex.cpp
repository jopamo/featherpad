...
static const QRegularExpression rExp(
    "/|\\b(?<!(@|#|%|\\$))(m|qr|q|qq|qw|qx|qr|(?<!-)s|y|tr)\\s*(?!\\=>)[^\\w\\}\\)\\]>\\s]");
...
bool Highlighter::isEscapedPerlRegex(const QString& text, const int pos) {
    ...
    if (i >= 0) {
        QChar ch = text.at(i);
        if (format(i) != regexFormat &&
            (ch.isLetterOrNumber() || ch == '_' || ch == ')' || ch == ']' || ch == '}' || ch == '#' ||
             (i == pos - 1 && (ch == '$' || ch == '@')) ||
             (i >= 1 && (text.at(i - 1) == '$' || (text.at(i - 1) == '@' && (ch == '+' || ch == '-')) ||
                         (text.at(i - 1) == '%' && (ch == '+' || ch == '-' || ch == '!'))))
             /* after an escaped start quote */
             || (i > 0 && (ch == '\"' || ch == '\'' || ch == '`') && format(i) != quoteFormat &&
                 format(i) != altQuoteFormat))) {
...
int Highlighter::findDelimiter(const QString& text,
                               const int index,
                               const QRegularExpression& delimExp,
                               int& capturedLength) const {
    int i = std::max(index, 0);
    const QString pattern = delimExp.pattern();
    if (pattern.startsWith("\\") && (pattern.endsWith(")") || pattern.endsWith("}") || pattern.endsWith("]") ||
                                     pattern.endsWith(">"))) {  // the end delimiter should be found
        int N = 1;
        if (index < 0) {
        ...
        QChar startBrace = endBrace == ')' ? '(' : endBrace == '}' ? '{' : endBrace == ']' ? '[' : '<';
...
bool Highlighter::isInsidePerlRegex(const QString& text, const int index) {
    ...
    if (pos == -1) {
        if (previousBlockState() != regexState && previousBlockState() != regexExtraState &&
    ...
    while ((nxtPos = findDelimiter(text, pos + 1, exp, capturedLength)) >= 0) {
        ...
        if (N % 2 == 0) {
            ...
            if (fi == commentFormat || fi == urlFormat || fi == quoteFormat || fi == altQuoteFormat ||
                fi == urlInsideQuoteFormat) {
                pos = nxtPos;  // don't add capturedLength because "/" might match later
                continue;
            }
            if (!searchedToReplace) {
                isQuotingOperator = (text.at(nxtPos) == 'q' && (capturedLength < 2 || text.at(nxtPos + 1) != 'r'));
                startPos = nxtPos + capturedLength;
            }
        }

        ++N;
        if (N % 2 == 0 ? isEscapedRegexEndSign(text, startPos < 0 ? std::max(0, pos + 1) : startPos, nxtPos,
                                               replacing || isQuotingOperator)  // an escaped end delimiter
                       : (capturedLength > 1
                              ? isEscapedPerlRegex(text, nxtPos)  // an escaped start sign
                              : (searchedToReplace
                                     ? isEscapedRegexEndSign(text, startPos < 0 ? std::max(0, pos + 1) : startPos,
...
void Highlighter::multiLinePerlRegex(const QString& text) {
    ...
    if (prevState == regexState || prevState == regexExtraState || prevState == regexSearchState) {
        bool between = false;  // between search and replacement, like s(...)HERE(...)
        QTextBlock prevBlock = currentBlock().previous();
        if (prevBlock.isValid()) {
            if (TextBlockData* prevData = static_cast<TextBlockData*>(prevBlock.userData())) {
        ...
        while (startIndex >= 0 && (isEscapedPerlRegex(text, startIndex) || fi == commentFormat || fi == urlFormat ||
        ...
        if (startIndex >= 0) {
            ...
            afterHereDocDelimiter =
                ((currentBlockState() >= endState || currentBlockState() < -1) && currentBlockState() % 2 == 0);
        }
    }
    ...
    while (startIndex >= 0) {
        ...
        bool continued(startIndex == 0 &&
                       (prevState == regexState || prevState == regexExtraState || prevState == regexSearchState));

        endExp.setPattern("\\" + getEndDelimiter(startDelimStr));
        int endLength;
        int endIndex = findDelimiter(text,
                                     continued ? -1  // to know that the search is continued from the previous line
        ...
        if (endIndex == -1) {
            if (!afterHereDocDelimiter)  // don't let an incorrect regex ruin a here-doc
            {
                ...
                static_cast<TextBlockData*>(currentBlock().userData())
                    ->insertInfo(ro ? "r" + startDelimStr : startDelimStr);
            ...
            if (continued) {
                if (prevState == regexSearchState) {
                    ...
                    if (getEndDelimiter(startDelimStr) != startDelimStr)  // regex replacement with braces
                    {
                        ...
                        if (startIndex == -1) {  // the line ends between search and replacement

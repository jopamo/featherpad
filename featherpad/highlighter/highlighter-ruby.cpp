...
bool Highlighter::isEscapedRubyRegex(const QString& text, const int pos) {
    ...
    if (i > -1) {
        ...
        if (ch == ')' || ch == ']' || ch == '}')
            return true;
        bool isString(false);
        if (ch.isNumber()) {  // check if this is really a number
            --i;
        ...
        /* as with Kate */
        if ((isString || ch.isLetter() || ch == '_') && pos + 1 < text.length() && text.at(pos + 1).isSpace()) {
            return true;
        }
    }
...
int Highlighter::findRubyDelimiter(const QString& text,
                                   const int index,
...
bool Highlighter::isInsideRubyRegex(const QString& text, const int index) {
    ...
    while ((nxtPos = findRubyDelimiter(text, pos + 1, exp, capturedLength)) >= 0) {
        ...
        ++N;
        if (N % 2 == 0 ? isEscapedChar(text, nxtPos)        // an escaped end delimiter
...
void Highlighter::multiLineRubyRegex(const QString& text) {
    ...
    if (prevState == regexState) {
        ...
        while (startIndex >= 0 && (isEscapedRubyRegex(text, startIndex) || fi == commentFormat || fi == urlFormat ||

...
bool Highlighter::isEscapedJavaQuote(const QString& text, const int pos, bool isStartQuote) const {
    if (pos <= 0)
        return false;
    if (isStartQuote) {  // the literal double quote character can be written as '"' or '\"'
        return (pos + 1 < text.length() && text.at(pos + 1) == '\'' &&
                (text.at(pos - 1) == '\'' || (pos - 2 >= 0 && text.at(pos - 1) == '\\' && text.at(pos - 2) == '\'')));
    }
    int i = 0;
...
/*************************/
bool Highlighter::isJavaSingleCommentQuoted(const QString& text, const int index, const int start) const {
    ...
    while ((nxtPos = text.indexOf(quoteMark, pos + 1)) >= 0) {
        ...
        if (fi == commentFormat || fi == urlFormat || fi == commentBoldFormat || fi == regexFormat) {
            pos = nxtPos;
            continue;
        }
...
/*************************/
bool Highlighter::isJavaStartQuoteMLCommented(const QString& text, const int index, const int start) const {
    if (index < 0 || start < 0 || index < start)
        return false;
...
void Highlighter::JavaQuote(const QString& text, const int start) {
    ...
    if (prevState != doubleQuoteState || index > 0) {
        ...
        while (isEscapedJavaQuote(text, index, true) || isJavaStartQuoteMLCommented(text, index)) {
    ...
    while (index >= 0) {
        ...
        while (isEscapedJavaQuote(text, index, true) || isJavaStartQuoteMLCommented(text, index, endIndex + 1)) {
            index = text.indexOf(quoteMark, index + 1);
        }
...
void Highlighter::multiLineJavaComment(const QString& text) {
    static const QRegularExpression javaDocTags(
        "@(author|deprecated|exception|param|return|see|serial|serialData|serialField|since|throws|version)|\\{@(code|"
        "docRoot|inheritDoc|link|linkplain|value)\\s[^\\}]*\\}");
    static const QRegularExpression javaDocBracedTags(
        "\\{@(code|docRoot|inheritDoc|link|linkplain|value)\\s[^\\}]*\\}");
    static const QRegularExpression descriptionEnd(
        "^\\s*\\**\\s*@(author|deprecated|exception|param|return|see|serial|serialData|serialField|since|throws|"
        "version)|"
        "\\.(?=\\s+|<br>|<p>|$)|<br>|<p>");
    static const QRegularExpression htmlTags(
        "</"
        ...
        "fieldset|figcaption|figure|font|footer|form|frame|frameset|h1|h2|h3|h4|h5|h6|head|header|hr|html|i|iframe|img|"
        "input|ins|kbd|label|legend|li|link|main|map|mark|meta|meter|nav|noframes|noscript|object|ol|optgroup|option|"
        "output|p|param|picture|pre|progress|q|rp|rt|ruby|s|samp|script|section|select|small|source|span|strike|strong|"
    ...
    while (startIndex >= 0) {
        ...
        if (prevState == commentState && startIndex == 0)
            ...
            endIndex = text.indexOf(commentEndExpression, startIndex + startMatch.capturedLength(), &endMatch);
...
void Highlighter::javaMainFormatting(const QString& text) {
...
#endif
    {
        ...
        if (rule.format != whiteSpaceFormat) {
            ...
            while (index >= 0 &&
                   (fi == quoteFormat || fi == urlInsideQuoteFormat || fi == commentFormat || fi == urlFormat ||
...
void Highlighter::javaBraces(const QString& text) {
    ...
    while (index >= 0 && (fi == quoteFormat || fi == altQuoteFormat || fi == urlInsideQuoteFormat ||
                          fi == commentFormat || fi == urlFormat || fi == commentBoldFormat || fi == regexFormat)) {

...
PrefDialog::PrefDialog(QWidget* parent) : QDialog(parent), ui(new Ui::PrefDialog) {
    ...
    if (ag.isEmpty())
        ag = QSize(std::max(700, config.getStartSize().width()), std::max(500, config.getStartSize().height()));
    ...
    if (FPwin* win = static_cast<FPwin*>(parent_)) {
        if (DEFAULT_SHORTCUTS.isEmpty()) {  // NOTE: Shortcut strings should be in the PortableText format.
            ...
            while (iter != defaultShortcuts.constEnd()) {
                ...
                OBJECT_NAMES.insert(
                    iter.key()->text().remove(QRegularExpression("\\s*\\(&[a-zA-Z0-9]\\)\\s*")).remove("&"), name);
                ++iter;
            }
        }
    }
    ...
    while (iter != OBJECT_NAMES.constEnd()) {
        shortcuts_.insert(iter.key(),
                          keys.contains(iter.value()) ? ca.value(iter.value()) : DEFAULT_SHORTCUTS.value(iter.value()));
        ++iter;
    }
    ...
    while (it != shortcuts_.constEnd()) {
        ...
        ui->tableWidget->setItem(
            ...
            new QTableWidgetItem(
                QKeySequence(it.value(), QKeySequence::PortableText).toString(QKeySequence::NativeText)));
        ++it;
        ++index;
    }
    ...
    if (syntaxNames.isEmpty()) {  // it's a shame that QObject::tr() doesn't work in FeatherPad::Config
    ...
    while (sIter != origSyntaxColors_.constEnd()) {
        ...
        label->setStyleSheet(
            QString("QLabel {background-color: rgb(%1, %2, %3);}").arg(col.red()).arg(col.green()).arg(col.blue()));
    ...
    connect(ui->syntaxTableWidget, &QWidget::customContextMenuRequested, [this](const QPoint& p) {
        ...
        connect(action, &QAction::triggered, this, [this, index] { changeSyntaxColor(index.row(), 1); });
...
void PrefDialog::showPrompt(const QString& str, bool temporary) {
        ...
        if (temporary)  // show it temporarily
        {
            if (!promptTimer_) {
                ...
                connect(promptTimer_, &QTimer::timeout, [this] {
                    if (!prevtMsg_.isEmpty() && ui->tabWidget->currentIndex() == 3)  // Shortcuts page
    ...
    else if (darkBg_ != config.getDarkColScheme() || (darkBg_ && darkColValue_ != config.getDarkBgColorValue()) ||
             (!darkBg_ && lightColValue_ != config.getLightBgColorValue()) ||
             showWhiteSpace_ != config.getShowWhiteSpace() || showEndings_ != config.getShowEndings() ||
             textMargin_ != config.getTextMargin() || textTabSize_ != config.getTextTabSize() ||
             (vLineDistance_ * config.getVLineDistance() < 0 ||
              (vLineDistance_ > 0 && vLineDistance_ != config.getVLineDistance())) ||
             whiteSpaceValue_ != config.getWhiteSpaceValue() ||
             disableMenubarAccel_ != config.getDisableMenubarAccel() || sysIcons_ != config.getSysIcons() ||
             curLineHighlight_ != config.getCurLineHighlight() ||
             origSyntaxColors_ != (!config.customSyntaxColors().isEmpty() ? config.customSyntaxColors()
                                   : config.getDarkColScheme()            ? config.darkSyntaxColors()
                                                                          : config.lightSyntaxColors())) {
...
bool PrefDialog::eventFilter(QObject* object, QEvent* event) {
    if (event->type() == QEvent::Wheel && (qobject_cast<QSpinBox*>(object) || qobject_cast<QComboBox*>(object)) &&
...
void PrefDialog::prefApplySyntax() {
    ...
    if (addLnagBtn || removeLangBtn) {  // add or remove all language buttons based on the new settings
        for (int i = 0; i < singleton->Wins.count(); ++i)
...
void PrefDialog::prefApplyDateFormat() {
    ...
    for (int i = 0; i < singleton->Wins.count(); ++i) {
        int count = singleton->Wins.at(i)->ui->tabWidget->count();
        for (int j = 0; j < count; ++j) {
            qobject_cast<TabPage*>(singleton->Wins.at(i)->ui->tabWidget->widget(j))->textEdit()->setDateFormat(format);
        }
    }
}
/*************************/
...
void PrefDialog::prefDarkColScheme(int checked) {
    ...
    disconnect(ui->whiteSpaceSpin, QOverload<int>::of(&QSpinBox::valueChanged), this,
               &PrefDialog::changeWhitespaceValue);
    ui->whiteSpaceSpin->setMinimum(config.getMinWhiteSpaceValue());
    ui->whiteSpaceSpin->setMaximum(config.getMaxWhiteSpaceValue());
...
void PrefDialog::prefThickCursor() {
    ...
    for (int i = 0; i < singleton->Wins.count(); ++i) {
        int count = singleton->Wins.at(i)->ui->tabWidget->count();
        for (int j = 0; j < count; ++j) {
            TextEdit* textedit = qobject_cast<TabPage*>(singleton->Wins.at(i)->ui->tabWidget->widget(j))->textEdit();
...
void PrefDialog::prefSelHighlight() {
    ...
    for (int i = 0; i < singleton->Wins.count(); ++i) {
        ...
        for (int j = 0; j < count; ++j) {
            qobject_cast<TabPage*>(singleton->Wins.at(i)->ui->tabWidget->widget(j))
                ->textEdit()
                ->setSelectionHighlighting(selHighlighting);
        }
    }
}
/*************************/
...
void PrefDialog::prefRecentFilesKind() {
    ...
    if (config.getRecentOpened() != openedKind) {
        ...
        for (int i = 0; i < singleton->Wins.count(); ++i) {
            singleton->Wins.at(i)->ui->menuOpenRecently->setTitle(openedKind ? tr("&Recently Opened")
...
/*************************/
void PrefDialog::prefTextTabSize(int value) {  // textTabSize_ is updated but the config isn't
    if (value >= 2 && value <= 10) {
        textTabSize_ = value;
        showPrompt();
    }
        ...
        if (cw->layout()) {
            if (const auto label = qobject_cast<QLabel*>(cw->layout()->itemAt(0)->widget())) {
                ...
                if (color.isValid() && color != prevColor) {
                    ...
                    if (prefCustomSyntaxColors_ == (config.getDarkColScheme()
                                                        ? config.darkSyntaxColors()
                                                        : config.lightSyntaxColors()))  // no customization
                    {
                        prefCustomSyntaxColors_.clear();
                        ui->defaultSyntaxButton->setEnabled(config.getWhiteSpaceValue() !=
                                                                config.getDefaultWhiteSpaceValue() ||
                                                            config.getCurLineHighlight() != -1);
                    }
                    else

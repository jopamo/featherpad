...
FontDialog::FontDialog(const QFont& font, QWidget* parent) : QDialog(parent), ui(new Ui::FontDialog), font_(font) {
    ui->setupUi(this);
    int widthHint = sizeHint().width();  // before hiding one of combo boxes

    /* a coding font should be normal and have a fixed pitch */
    bool codingFont = !font_.italic() && font_.weight() == QFont::Normal && QFontInfo(font_).fixedPitch();
